...
function EntertainmentPageContent() {
    ...
    const updateURL = useCallback(
        (updates: { page?: number; q?: string }) => {
            ...
            const newURL = params.toString() ? `?${params.toString()}` : '/entertainment';
            ...
            try {
                ...
                if (searchKeyword.trim()) {
                    ...
                    const filteredSearchData = searchData.filter(
                        (item) => item.type === 'entertainment' && matchedIds.includes(item.id)
                    );
        ...
        <>
            <section className="w-full py-2">
                <div className="text-center mb-3">
                    <h2 className="text-xl font-bold mb-2">娱乐休闲</h2>
                    <p className="text-sm text-default-600">放松身心好去处</p>
                </div>
                    ...
                    {loading ? (
                        ...
                        restaurants.map((restaurant) => (
                            <Card
                                ...
                                className="w-full"
                                isPressable
                                onPress={() => router.push(`/entertainment/${restaurant.id}`)}
                            >
