...
async function seedJobsData() {
    ...
    try {
        ...
        for (const job of jobsData) {
            const createdJob = await prisma.parttime.create({
                data: job,
            });
        ...
        // 显示添加的数据
        const jobs = await prisma.parttime.findMany({
