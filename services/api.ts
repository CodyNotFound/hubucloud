...
export class DataService {
    // 兼职服务
    static parttime = {
        async getList(params: FilterParams = {}): Promise<ApiResponse<{ parttimes: Parttime[], pagination: any }>> {
    ...
    // 餐厅服务
    static restaurant = {
        async getList(params: FilterParams = {}): Promise<ApiResponse<{ restaurants: Restaurant[], pagination: any }>> {
            const query = new URLSearchParams();
            Object.keys(params).forEach((key) => {
                if (params[key] !== undefined && params[key] !== '') {
                    query.append(key, params[key].toString());
